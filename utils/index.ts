...
import { TOP_PAIRS, PAIRS_VOLUME_QUERY, TOKEN_BY_ADDRESS, PAIR_BY_ADDRESS } from "./apollo/queries";
...
export type PairData = PairQuery["pair"];
...
export async function getPairByAddress(address: string): Promise<PairData> {
  const {
    data: { pair },
    errors: tokenErrors,
